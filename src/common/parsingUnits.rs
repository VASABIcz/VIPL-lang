...
struct OperatorAssignment {
    ...
    pub typ: TokenType,
    pub priority: usize,
    inner: ArithmeticParsingUnit,
}

impl OperatorAssignment {
    pub fn new(op: ArithmeticOp, typ: TokenType, priority: usize) -> Self {
        Self {
            op,
            typ,
            priority,
...
impl ParsingUnit<ASTNode, TokenType, VIPLParsingState> for OperatorAssignment {
    ...
    fn parse(&self, parser: &mut Parser<TokenType, ASTNode, VIPLParsingState>) -> Result<ASTNode, ParserError<TokenType>> {
        let idk = if parser.isPrevAssignable() && parser.tokens.isPeekIndexType(Equals, 1) {
            parser.parseWrappedStatement(|parser| {
                let prev = parser.prevPop()?.asExpr()?;
                parser.tokens.getAssert(self.typ)?;
                parser.tokens.getAssert(TokenType::Equals)?;
                let expr = parser.parseExpr()?;

                Ok(RawStatement::Assignable(prev, expr, Some(self.op), None))
            })
        } else {
                    ...
                    if self.priority < p.getPriority() {
                        ...
                        xd.getMut().previousBuf.push(
                            ASTNode::Expr(Expression {
                                exp: RawExpression::BinaryOperation {
                                    left: Box::new(prev),
                                    right: Box::new(res.asExpr()?),
                                    op: self.op.clone(),
                                },
        ...
        parser.parseWrappedExpression(|parser| {
            ...
            let res = match peek.typ {
                ...
                TokenType::FloatLiteral => {
                    buf.push_str(&peek.str);
                    (RawExpression::FloatLiteral(buf))
                }
...
fn parsingUnits() -> Vec<Box<dyn ParsingUnit<ASTNode, TokenType, VIPLParsingState>>> {
    ...
    vec![
        ...
        Box::new(OperatorAssignment::new(
            ArithmeticOp::Mul,
            TokenType::Mul,
            {
                x += 1;
