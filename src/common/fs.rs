...
pub fn setupFs(vm: &mut VirtualMachine) {
    vm.makeNative("ls".to_string(), Box::new([VariableMetadata { name: "".to_string(), typ: DataType::Str() }]), |vm, locals| {
    ...
    vm.makeNative("readFile".to_string(), Box::new([VariableMetadata { name: "".to_string(), typ: DataType::Str() }]), |vm, locals| {
        let path = locals.localVariables.first().unwrap().getString();
    ...
    vm.makeNative("fileType".to_string(), Box::new([VariableMetadata { name: "".to_string(), typ: DataType::Str() }]), |vm, locals| {
        let path = locals.localVariables.first().unwrap().getString();
